...
func Provision(opts Options) error {
	...
	cfgPath, cleanup, err := writeDeviceConfig(ssid, password, opts.BrokerHost, opts.BrokerPort, opts.PixelPin, opts.ProvisionTag)
	if err != nil {
		return err
	}
	defer cleanup()
...
func writeDeviceConfig(ssid, password, broker string, port, pixelPin int, provisionTag string) (string, func(), error) {
	content := fmt.Sprintf("# Generated by backend provisioning\nWIFI_SSID = %q\nWIFI_PASSWORD = %q\nMQTT_BROKER = %q\nMQTT_PORT = %d\nPIXEL_PIN = %d\nPIXEL_COUNT = 255\nPROVISION_TAG = %q\n", ssid, password, broker, port, pixelPin, provisionTag)
